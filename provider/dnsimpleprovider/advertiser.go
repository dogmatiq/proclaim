...
func (d *changeSet) Delete(rec dnsimple.ZoneRecord) {
	d.delete = append(d.delete, rec.ID)
}
...
func (a *advertiser) applyChangeSet(
	...
	for _, id := range cs.delete {
		if _, err := a.API.DeleteRecord(ctx, accountID, a.Zone.Name, id); err != nil {
			return 0, 0, 0, fmt.Errorf("unable to delete zone record: %w", err)
		}
	}

	for _, u := range cs.update {
		if _, err := a.API.UpdateRecord(ctx, accountID, a.Zone.Name, u.ID, u.Attr); err != nil {
			return 0, 0, 0, fmt.Errorf("unable to update zone record: %w", err)
		}
	}

	for _, r := range cs.create {
		if _, err := a.API.CreateRecord(ctx, accountID, a.Zone.Name, r); err != nil {
			return 0, 0, 0, fmt.Errorf("unable to create zone record: %w", err)
		}
	}
